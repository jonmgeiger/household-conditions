...
school_assess <- read_csv("../data/school_assess.csv")
grad_raceP_household_rev <- read_csv("../data/grad_raceP_household_rev.csv")


```

## Average assessment scores 2014-2018 

Lets calculate the average reading and math assessment scores for each district over 5 years. 
...
```{r}
 district_assess_corr <- district_assess %>%
  select(grad_rate_midpt, pct_pov:pct_CLI, read_score, math_score) %>%
  na.omit()
...
```{r}
 district_assess_corr_race <- district_assess %>%
  select(grad_rate_midpt, pct_hisp_latino:pct_PI, read_score, math_score) %>%
  na.omit()
